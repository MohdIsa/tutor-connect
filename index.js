...
app.post('/:userType/login', function (req, res) {
  ...
  pool.query(qrystr, function (error, results, fields) {
    ...
    } else res.send('<h3>Not A User</h3>');
  });
});
...
app.post('/:usertype/register', function (req, res) {
  ...
  function UpdateLogin() {
    ...
    pool.query(qrystr, function (error, results, fields) {
      ...
      else console.log('Login Table Updated');
    });
  }
  ...
  if (userType == 'student') {
    ...
    qrystr =
      'INSERT INTO Student(F_Name,L_Name,Email,City,Contact) VALUES ("' + req.body.fname + '","' + req.body.lname + '","' + req.body.email + '","' + req.body.city + '",' + req.body.contact + ')';
    console.log(qrystr);
    pool.query(qrystr, function (error, results, fields) {
      if (error) {
    ...
    qrystr =
      'SELECT S_ID FROM Student WHERE F_Name= "' +
      req.body.fname +
      '"AND L_Name= "' +
      req.body.lname +
      '"AND Email= "' +
  ...
  if (userType == 'tutor') {
    ...
    qrystr =
      'Insert into Tutor (F_Name, L_Name, Email, Phone, City, Qualification, Experience) VALUES ("' +
      req.body.fname +
      '", "' +
      req.body.lname +
      '","' +
  ...
  if (userType == 'coaching') {
    // console.log(req.body);
    qrystr =
      'Insert into Coaching(Name, Email, City, Contact, Establishment) VALUES(' +
      '"' +
      req.body.name +
      '",' +
      '"' +
...
app.post('/:usertype/:userid/defaultPreferences', function (req, res) {
  ...
  if (userType == 'tutor') {
    for (var i = 0; i < req.body.class.length; i++) {
      ...
      pool.query(qrystr, function (error, results, fields) {
        ...
        else console.log('Record added in CoursePool table!');
      });
  ...
  if (userType == 'coaching') {
    for (var i = 0; i < req.body.class.length; i++) {
      ...
      pool.query(qrystr, function (error, results, fields) {
        if (error) console.log('Duplicate entry!');
        else console.log('Record added in Courses table!');
      });
      pool.query(qrystr1, function (error, results, fields) {
        if (error) throw error;
  ...
  if (userType == 'student') {
    ...
    pool.query(qrystr, function (err, result) {
      ...
      pool.query(qrystr1, function (err, result) {
        if (err) throw err;
        else console.log('courses query');
        console.log(result);
        var courses = result;
        //console.log(sbasics[0].City);
        var qrystr2 =
          'SELECT * from Tutor a inner join T_Courses b on a.T_ID = b.T_ID inner join CoursePool c on b.Course_ID = c.Course_ID where a.City in (select City from Student where S_ID = ' +
          userId +
          ') and b.Course_ID IN (select Course_ID from S_Courses where S_ID=' +
          userId +
          ')';
        pool.query(qrystr2, function (error, result, fields) {
          ...
          var tutors = result;
          var qrystr3 =
            'SELECT * from Coaching a inner join C_Courses b on a.C_ID = b.C_ID inner join CoursePool c on b.Course_ID = c.Course_ID where a.City in (select City from Student where S_ID = ' +
            userId +
            ') and b.Course_ID IN (select Course_ID from S_Courses where S_ID=' +
            userId +
            ')';
...
app.get('/:usertype/:userid/delete', function (req, res) {
  ...
  if (userType == 'student') {
    ...
    pool.query(qrystr, function (error, results, fields) {
      if (error) throw error;
      else console.log('deleted student');
      res.redirect('/');
    });
  }

  if (userType == 'tutor') {
    var qrystr = 'DELETE FROM Tutor where T_ID=' + userId;
    pool.query(qrystr, function (error, results, fields) {
      if (error) throw error;
      else console.log('deleted tutor');
      res.redirect('/');
    });
  }
  if (userType == 'coaching') {
    var qrystr = 'DELETE FROM Coaching where C_ID=' + userId;
    pool.query(qrystr, function (error, results, fields) {
      if (error) throw error;
      else console.log('deleted coaching');
      res.redirect('/');
    });
  }
});
...
app.get('/:usertype/:userid/resetprefs', function (req, res) {
  ...
  if (userType == 'student') {
    ...
    pool.query(qrystr, function (error, results, fields) {
      if (error) throw error;
      else console.log('Reset student preferences');
  ...
  if (userType == 'tutor') {
    ...
    pool.query(qrystr, function (error, results, fields) {
      if (error) throw error;
      else console.log('Reset tutor preferences');
  ...
  if (userType == 'coaching') {
    ...
    pool.query(qrystr, function (error, results, fields) {
      if (error) throw error;
      else console.log('Reset coaching preferences');
